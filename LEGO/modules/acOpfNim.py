...
def add_element_definitions_and_bounds(model: pyo.ConcreteModel, cs: CaseStudy) -> (list[pyo.Var], list[pyo.Var]):
    ...
    model.vLineP = pyo.Var(model.rp, model.k, model.la_full, doc='Power flow from bus i to j', bounds=lambda m, rp, k, i, j, c: (-m.pPmax[i, j, c], m.pPmax[i, j, c]) if (i, j, c) in m.la else (-m.pPmax[j, i, c], m.pPmax[j, i, c]))
    ...
    model.vQNS = pyo.Var(model.rp, model.k, model.i, doc='Slack variable reactive power not served', bounds=lambda model, rp, k, i: (0, max(model.pDemandQ[rp, k, i], 0)))
...
def add_constraints(model: pyo.ConcreteModel, cs: CaseStudy):
    ...
    def eSOCP_VoltageLimitSlack_rule1(m, rp, k, i):
        return (m.vSOCP_cii[rp, k, i] >= m.pBusMinV[i] ** 2)
    def eSOCP_VoltageLimitSlack_rule2(m, rp, k, i):
        return (m.vSOCP_cii[rp, k, i]<= m.pBusMaxV[i] ** 2)
