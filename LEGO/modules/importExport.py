...
def add_element_definitions_and_bounds(lego: LEGO):
    ...
    # Variables
    lego.model.vImport = pyo.Var(lego.model.importNodes, lego.model.rp, lego.model.k, doc='Import at node i', bounds=(0, None))
    lego.model.vExport = pyo.Var(lego.model.exportNodes, lego.model.rp, lego.model.k, doc='Export at node i', bounds=(0, None))
    for rp in lego.model.rp:
        for k in lego.model.k:
            for i in lego.model.importNodes:
                if any(lego.cs.dPower_ImpExp.index.isin([(i, rp, 'ImportFix', k)])):
                    lego.model.vImport[i, rp, k].fix(lego.cs.dPower_ImpExp.loc[(i, rp, 'ImportFix', k)].iloc[0])
                elif any(lego.cs.dPower_ImpExp.index.isin([(i, rp, 'ImportMax', k)])):
                    lego.model.vImport[i, rp, k].setub(lego.cs.dPower_ImpExp.loc[(i, rp, 'ImportMax', k)].iloc[0])
                else:
                    lego.model.vImport[i, rp, k].fix(0)

                if any(lego.cs.dPower_ImpExp.index.isin([(i, rp, 'ExportFix', k)])):
                    lego.model.vExport[i, rp, k].fix(lego.cs.dPower_ImpExp.loc[(i, rp, 'ExportFix', k)].iloc[0])
                elif any(lego.cs.dPower_ImpExp.index.isin([(i, rp, 'ExportMax', k)])):
                    lego.model.vExport[i, rp, k].setub(lego.cs.dPower_ImpExp.loc[(i, rp, 'ExportMax', k)].iloc[0])
                else:
                    lego.model.vExport[i, rp, k].fix(0)

    # Parameters
    lego.model.pImpExpPrice = pyo.Var(lego.model.importNodes | lego.model.exportNodes, lego.model.rp, lego.model.k, doc='Imp-/Export price at node i',
                                      initialize=lego.cs.dPower_ImpExp[lego.cs.dPower_ImpExp.index.isin(['Price'], level=2)].droplevel("Type"))
